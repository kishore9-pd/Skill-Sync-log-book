...
async function handleRegisterSubmit(e) {
  ...
  try {
    const res = await fetch('/api/register', {
      ...
      body: JSON.stringify({ name, email, combo, password })
    });
    ...
    const user = { name, email, combo, password };
...
function enterMainApp() {
  ...
  const topSelect = document.getElementById('comboSelect');
  if (topSelect) topSelect.value = appState.activeCombo;

  document.getElementById('userComboBadge').innerText = (appState.comboConfigs[appState.activeCombo]?.name) || appState.activeCombo;
